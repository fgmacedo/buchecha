...
- **stdlib only** for everything else: `os/exec`, `encoding/json`, `bufio`, `context`, `log/slog`, `text/template`, `net/http` (for the MCP server).
...
internal/mcp/                    (stdlib HTTP MCP server with per-connection
                                  auth; the run boot starts one per `bcc run`)
        ↑
...
1. `internal/mcp/` is a stdlib-only HTTP MCP server with per-connection authorization (the run boot supplies a Handler that validates `agent_id` and role on every dispatch). It depends on nothing else in the project.
1. `internal/git/cli/` shells out to `git` via `os/exec`. Anything else that needs git later goes through the same port.
1. `internal/tui/` imports `loop` types, `director` types, and `agentcontract` types. Never the reverse.
1. `internal/cli/` wires everything: load config → start MCP boot → resolve session → build adapters → build loop → run.
