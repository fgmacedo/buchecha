...
1. [x] **Plumb `agentcontract.ParseLine` in the executor.** Hook in `internal/executor/claude/claude.go` routes `bcc_event` lines through `agentcontract.ParseLine`; `loop.AgentEvent` gained `KindBccEvent` and a `Bcc *agentcontract.BccEvent` field; NDJSON serialiser handles the new shape.
...
Single substantial commit migrated `Loop.Run`, `LoopDecider`, the TUI panels, and `cli/run.go` from the parser-based path to the wire-protocol-first path; a follow-up commit deleted `internal/spec/`, `internal/specreader/markdown/`, `internal/loop/prompt.go`, and `docs/guides/autonomous-execution.md`. Three ports remain: `Executor`, `GitProbe`, `AgentBriefing`. CLAUDE.md updated.

- **Decisions**: bcc no longer reads any user-space artifact at runtime. The legacy `[executor]` config block survives for one more iteration (next commit reshapes it into `[agent.<name>]` per the hierarchical config plan). `cfg.Specs.PlanHeading` / `JournalHeading` still flow into `markdown_bcc.Config` as template inputs, even though they no longer drive any parsing on the bcc side; the rename to `[spec.markdown_bcc]` happens in the config-hierarchy commit. The TUI's progress panel display switched from "X/Y items" (parsed plan) to "X/Y tasks" (cumulative `task_started`/`task_completed` from the wire). Existing tests for parser-based paths were rewritten in place; `loop_test.go` and `integration_test.go` now drive the loop with scripted `bcc_event` streams via the fake executor.
