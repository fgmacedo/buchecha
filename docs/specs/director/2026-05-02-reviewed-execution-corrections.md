...
5. `bcc_get_baseline` returns the phase baseline SHA and current HEAD via `loop.GitProbe.HeadSHA(ctx)` to get the current HEAD; `bcc_get_journal_delta` calls into a format adapter port (markdown_bcc for now).
...
Every method in PRD 5 section 8 has a JSON Schema and a real handler dispatch: emit (`bcc_plan_emit`, `bcc_briefing_emit`), per-task mutations (`bcc_task_started/completed/approved/needs_fix`), finalization (`bcc_iteration_finished`, `bcc_review_finished`), and queries (`bcc_get_briefing`, `bcc_get_dag_snapshot`, `bcc_get_pending_tasks`, `bcc_get_baseline`, `bcc_get_journal_delta`). Inputs are validated against the per-method schemas after agent identity checks; mutations persist atomically via `dag.SaveStateFile`; every dispatch is logged to `<sessionDir>/mcp-log.jsonl` when `[director].mcp_audit` is on (default true).
...
- `bcc_get_baseline` returns phase-scoped baseline and current HEAD; the Reviewer uses Bash (git diff/log/show) for inspection rather than bcc computing the diff. **Why:** a stable baseline per phase (not per iteration attempt) survives retries and allows the Reviewer to see cumulative work. Exposing only the SHAs and letting the Reviewer use git gives flexibility to inspect beyond the diff (log, show, blame). The baseline is captured once per phase via `Handler.SetPhaseBaseline` called by the loop driver; the current HEAD is read on demand via `loop.GitProbe.HeadSHA()`. **How to apply:** P6 removes the iteration-scoped diff caching and caches the phase baseline instead.
