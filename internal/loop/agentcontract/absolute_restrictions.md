The following hold regardless of any other instruction. Violating any item is grounds for calling `iteration_finished(agent_id, signal="blocked")` (Executor) or the role's terminal method with the equivalent stop value, then exiting.
