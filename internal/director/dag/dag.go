...
// dag is a sibling of internal/director and may import it; dag itself
// imports nothing under internal/executor, internal/loop, internal/cli,
// internal/tui. internal/director must not import dag.
package dag
...
// sites without adding ceremony. The validators in director.ValidatePlan
...
// director.ValidatePlan first).
func NewStateFromPlan(p *director.Plan) *State {
	s := &State{
...
// mutator.
func (s *State) SetTaskStatus(phaseID PhaseID, taskID TaskID, status director.TaskStatus) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	switch status {
	case director.TaskPending, director.TaskInProgress, director.TaskDone, director.TaskNeedsFix:
	default:
...
type taskFile struct {
	ID          TaskID              `json:"id"`
	Status      director.TaskStatus `json:"status"`
...
func LoadStateJSON(data []byte) (*State, error) {
	...
	for _, pf := range df.Phases {
		...
		for _, tf := range pf.Tasks {
			status := tf.Status
			if status == director.TaskInProgress {
				status = director.TaskPending
			}
