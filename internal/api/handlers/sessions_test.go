...
// helper. The shape is the canonical session.Session JSON form.
...
func twoSessionsServer(t *testing.T) (*httptest.Server, session.Session, session.Session) {
	t.Helper()
	tmp := t.TempDir()
...
func TestSessions_ListIncludesLiveAndArchived(t *testing.T) {
	...
	if body.Sessions[0].Status != string(session.SessionRunning) {
		t.Errorf("live status: got %q, want %q", body.Sessions[0].Status, session.SessionRunning)
	}
	if body.Sessions[1].Status != string(session.SessionDone) {
		t.Errorf("archived status: got %q, want %q", body.Sessions[1].Status, session.SessionDone)
	}
}
...
func TestSessions_GetLive(t *testing.T) {
	...
	if meta.Status != string(session.SessionRunning) {
		t.Errorf("status: got %q, want %q", meta.Status, session.SessionRunning)
	}
}
...
func TestSessions_GetArchived(t *testing.T) {
	...
	if meta.Status != string(session.SessionDone) {
		t.Errorf("status: got %q, want %q", meta.Status, session.SessionDone)
	}
}
