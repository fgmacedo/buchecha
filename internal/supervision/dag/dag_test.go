		...
		Phases: []supervision.Phase{
			{
				...
				Tasks: []supervision.Task{
					{ID: "t1", Status: supervision.TaskPending, RetryBudget: 1},
					{ID: "t2", Status: supervision.TaskPending, DependsOn: []string{"t1"}, RetryBudget: 2},
				},
			},
			{
				ID:        "P2",
...
func TestEligiblePhases_P2EligibleAfterP1Done(t *testing.T) {
	s := NewStateFromPlan(samplePlan())
	if err := s.SetTaskStatus("P1", "t1", supervision.TaskDone); err != nil {
		t.Fatal(err)
	}
	if err := s.SetTaskStatus("P1", "t2", supervision.TaskDone); err != nil {
		t.Fatal(err)
	}
	got := s.EligiblePhases()
...
func TestSetTaskStatus_RejectsUnknownIDs(t *testing.T) {
	s := NewStateFromPlan(samplePlan())
	if err := s.SetTaskStatus("PX", "t1", supervision.TaskDone); err == nil {
		t.Error("expected error for unknown phase")
	}
	if err := s.SetTaskStatus("P1", "tX", supervision.TaskDone); err == nil {
		t.Error("expected error for unknown task")
	}
	if err := s.SetTaskStatus("P1", "t1", supervision.TaskStatus("weird")); err == nil {
...
func TestState_ConcurrentApplyIsRaceFree(t *testing.T) {
	...
	mutators := []func() error{
		func() error { return s.SetTaskStatus("P1", "t1", supervision.TaskInProgress) },
		func() error { return s.SetTaskStatus("P1", "t1", supervision.TaskDone) },
		func() error { return s.SetTaskStatus("P1", "t2", supervision.TaskInProgress) },
		func() error { return s.SetTaskStatus("P1", "t2", supervision.TaskDone) },
		func() error { return s.SetTaskStatus("P2", "t1", supervision.TaskNeedsFix) },
		func() error { return s.SetTaskStatus("P2", "t1", supervision.TaskPending) },
	}
	for i := 0; i < 200; i++ {
...
func TestState_SnapshotRoundTrip(t *testing.T) {
	...
	if err := s.SetTaskStatus("P1", "t2", supervision.TaskInProgress); err != nil {
		t.Fatal(err)
	}
...
func TestState_LoadReconcilesInProgressOnDisk(t *testing.T) {
	...
	if got.Phase("P2").Tasks["t1"].Status != supervision.TaskDone {
