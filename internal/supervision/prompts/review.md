...
1. Call `task_started(agent_id, "reviewing")` so the timeline records that review began.
2. Call `get_briefing(agent_id)` to retrieve the briefing the Executor was given. It pins `phase_id`, `sub_dag_task_ids`, `instructions`, and `spec_path`.
3. Call `get_dag_snapshot(agent_id)` for current per-task status across the audited phase.
4. Call `get_baseline(agent_id)` for the phase-scoped baseline. The handler returns `{phase_id, phase_baseline_sha, current_head_sha}`; combine these with `Bash` to inspect what the Executor produced:
...
5. Call `get_journal_delta(agent_id)`. Empty delta is fine: specs without an Execution Journal carry no journaling surface, and the Executor is instructed to skip in that case.
...
7. For each task in `sub_dag_task_ids`, walk its `acceptance` list and decide pass/fail per item:
   ...
   - `evidence: manual`: you cannot execute it. Mark the task as `needs_fix` only if the diff alone proves divergence; otherwise approve and surface the criterion in `review_finished` reasoning.
8. Per task, call exactly one of:
   - `task_approved(agent_id, task_id, note?)` when every acceptance item passes.
   - `task_needs_fix(agent_id, task_id, feedback)` with a terse, actionable feedback string when at least one acceptance item fails. Feedback rides into the next iteration's `prior_feedback`; phrase it as something the Executor can act on directly.
9. Close with `review_finished(agent_id, outcome, reasoning?)` where outcome is:
...
10. Call `task_completed(agent_id, "reviewing", summary)` once the verdict is in. `summary` is one short sentence describing the call (e.g. "phase P5 sub-DAG = T5.4, T5.5; outcome=approve; all acceptance bullets satisfied").
